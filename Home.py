...
def load_submissions():
    """Reads submissions from the local JSON file."""
...
def save_submission(record):
    """Saves a new submission to the local JSON file."""
...
def faculty_page(df):
    ...
    difficulties = st.text_area("Difficulties and Suggestions regarding academic assignments:")
    
    # --- Final Submission ---
    st.divider()
    st.checkbox("I confirm that the information provided above is accurate.")
    ...
    with submit_col2:
        if st.button("🚀 Submit Complete Appraisal Form", type="primary", use_container_width=True):
            ...
            submission_record = {
                "Faculty Name": st.session_state.username,
                "Date Submitted": datetime.now().strftime("%Y-%m-%d %H:%M"),
                "Total Score": int(total_score), # Cast to int so JSON can serialize it safely
                "Rating": rating,
                "SCI Papers": int(selected_sci),
                "Scopus Papers": int(selected_scopus),
                "Projects": len(edited_projects),
                "Target SCI Next Year": target_sci,
                "Feedback": difficulties
            }
            
            # Save to JSON file instead of session state
            save_submission(submission_record)
            
            st.success("🎉 Appraisal Submitted Successfully! You can now log out. The Admin will see it immediately.")
            st.balloons()
...
def admin_page(df):
    ...
    if len(all_submissions) == 0:
        ...
        st.subheader(f"📋 Faculty Submissions Overview ({len(all_submissions)} Total)")
        sub_df = pd.DataFrame(all_submissions)
        st.dataframe(sub_df[["Faculty Name", "Date Submitted", "Total Score", "Rating"]], use_container_width=True)
